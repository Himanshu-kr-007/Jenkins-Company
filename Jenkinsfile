pipeline {
    ...
    stages {
        ...
        //             sh "ssh -o StrictHostKeyChecking=no ec2-user@13.127.165.100 sudo  docker rm -f mywebserver"
        //             sh "ssh -o StrictHostKeyChecking=no ec2-user@13.127.165.100 sudo docker run -d -p 80:80 --name mywebserver docker.io/himanshukr0612/webserver:${BUILD_TAG}"
        //         }
        //     }
        // }
        stage('Testing Phase'){
            steps{
                sh 'curl -sleep http://13.127.165.100:8081/ | grep DevOps'
                sh 'curl -sleep http://13.127.165.100:8081/ | grep Jenkins'
            }
        }
        ...
        stage('Deploy In Prod'){
            steps{
                sshagent(['Jenkins-Main']) {
                    sh "ssh -o StrictHostKeyChecking=no ec2-user@13.127.165.100 sudo docker rm -f prod"
                    sh "ssh -o StrictHostKeyChecking=no ec2-user@13.127.165.100 sudo docker run -d -p 80:80 --name prod docker.io/himanshukr0612/webserver:${BUILD_TAG}"
                }
            }
        }
        stage('LIVE'){
